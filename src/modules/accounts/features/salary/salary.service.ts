...
export class SalaryService {
  ...
  async detail(id: string) {
    const data = await this.prisma.salaryProcessMst.findUnique({
      where: { id: BigInt(id) },
      include: {
        salaryProcessDtls: { include: { employee: true } },
        shop: true,
      },
    });
  ...
  async save(data: Record<string, any>) {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      const masterPayload: any = {
        process_no:
          data.process_no || `SAL-${randomUUID().slice(0, 8).toUpperCase()}`,
        ...
        total_paid_amount: data.total_paid_amount
          ? Number(data.total_paid_amount)
          : 0,
        total_due_amount: data.total_due_amount
          ? Number(data.total_due_amount)
          : 0,
      ...
      if (id) {
        await tx.salaryProcessDtl.deleteMany({
          where: { salary_process_mst_id: master.id },
        });
      }
    ...
    return {
      message: id
        ? 'Salary process updated successfully'
